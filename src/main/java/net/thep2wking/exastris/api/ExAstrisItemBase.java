...
public class ExAstrisItemBase extends Item implements IHasModel {
    ...
    public ExAstrisItemBase(String name, CreativeTabs tab, EnumRarity rarity, boolean hasEffect, int tooltipLines) {
        this.name = name;
        this.tab = tab;
        this.rarity = rarity;
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        if (TooltipHandler.showShiftTip()) {
            for (int i = 1; i <= tooltipLines; ++i) {
                TooltipHandler.addShiftTooltip(tooltip, (ExAstris.PREFIX + this.name), i);
            }
